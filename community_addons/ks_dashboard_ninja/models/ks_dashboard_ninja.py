...
class KsDashboardNinjaBoard(models.Model):
    ...
    duplicate_ks_dashboard_top_menu_id = fields.Many2one('ir.ui.menu', domain="[('parent_id','=',False)]",
                                                         string="Duplicate Show Under Menu")

    duplicate_ks_dashboard_menu_id = fields.Many2one('ir.ui.menu')
    ...
    def create(self, vals):
        ...
        if 'ks_dashboard_top_menu_id' in vals and 'ks_dashboard_menu_name' in vals:
            ...
            parent_menu_id = self.env['ir.ui.menu'].sudo().browse(vals['ks_dashboard_top_menu_id'])
            duplicate_top_menu_id = parent_menu_id.copy()
            duplicate_top_menu_id.name = vals['name']

            duplicate_menuid = record.ks_dashboard_menu_id.copy()
            duplicate_menuid.parent_id, duplicate_menuid.name = duplicate_top_menu_id.id, record.ks_dashboard_menu_id.name

            record.duplicate_ks_dashboard_top_menu_id = duplicate_top_menu_id.id
            record.duplicate_ks_dashboard_menu_id = duplicate_menuid.id
            # hasan end
        ...
        for rec in self:
            if 'ks_dashboard_menu_name' in vals:
                if self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board') and self.env.ref(
                    ...
                    rec.duplicate_ks_dashboard_menu_id.sudo().name = vals['ks_dashboard_menu_name']
            if 'ks_dashboard_group_access' in vals:
                if self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board').id == rec.id:
                    ...
                    rec.duplicate_ks_dashboard_menu_id.sudo().groups_id = vals['ks_dashboard_group_access']
            if 'ks_dashboard_active' in vals and rec.ks_dashboard_menu_id:
                rec.ks_dashboard_menu_id.sudo().active = vals['ks_dashboard_active']
                rec.duplicate_ks_dashboard_menu_id.sudo().active = vals['ks_dashboard_active']
