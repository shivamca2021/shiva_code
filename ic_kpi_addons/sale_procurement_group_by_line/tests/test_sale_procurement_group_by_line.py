...
class TestSaleProcurementGroupByLine(TransactionCase):
    ...
    def test_procurement_group_by_line(self):
        ...
        self.picking_ids = self.env["stock.picking"].search([("group_id", "in", self.line2.procurement_group_id.ids)])
    ...
    def test_action_launch_procurement_rule_1(self):
        group_id = self.proc_group_model.create({"move_type": "one", "sale_id": self.sale.id, "name": self.sale.name})
    ...
    def test_action_launch_procurement_rule_3(self):
        ...
        self.assertEqual(self.line1.procurement_group_id, self.line2.procurement_group_id)
