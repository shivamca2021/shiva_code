...
class TestAccessMenu(TransactionCase):
    ...
    def test_access_menu(self):
        ...
        self.menu_settings.write({"groups_id": [(6, 0, [self.group_employee.id])]})
        ...
        self.menu_settings.write({"restrict_group_ids": [(6, 0, [self.group_employee.id])]})
        ...
        self.demo_user.write({"hidden_menu_ids": [(6, 0, [self.menu_ir_property.id])]})

        menu = self.Menu.sudo(self.demo_user.id).search([])

        # check menu 'Settings/Technical/Parameters' hidden for demo_user
        self.assertNotIn(self.menu_ir_property, menu)
        self.assertEqual(self.menu_ir_property.hide_from_user_ids.id, self.demo_user.id)
