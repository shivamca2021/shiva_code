...
ProductConfiguratorFormRenderer.include({
    ...
    renderConfigurator: function (configuratorHtml) {
        ...
        self.speciesProducts = [this.$el.find('select[name="species_ids"]').val()];
        this.$el.find('select[name="species_ids"]').click(function (event) {
            var species = event.currentTarget;
            if ($(event.currentTarget).val()) {
                self.speciesProducts = [$(event.currentTarget).val()];
            }
        ...
        self.productWidth = parseFloat(this.$el.find('input#product_width').val());
        this.$el.find('input#product_width').keyup(function(event) {
            var product_width = event.currentTarget;
            self.productWidth = parseFloat($(product_width).val());
            self.productMinWidth = self.$el.find('input#product_min_size').val();
            self.productMaxWidth = self.$el.find('input#product_max_size').val();
        });
        self.productHeight = parseFloat(this.$el.find('input#product_height').val());
        this.$el.find('input#product_height').keyup(function(event) {
            var product_height = event.currentTarget;
            self.productHeight = parseFloat($(product_height).val());
        });
        self.productDepth = parseFloat(this.$el.find('input#product_depth').val());
        this.$el.find('input#product_depth').keyup(function(event) {
            var product_depth = event.currentTarget;
            self.productDepth = parseFloat($(product_depth).val());
        });
    },
});
...
//        };
        ...
        result['product_width'] = mainProduct.product_width;
        result['product_height'] = mainProduct.product_height;
        result['product_depth'] = mainProduct.product_depth;
        return result;
    },
...
//                ),
                ...
                default_product_width: this.recordData.product_width,
                default_product_height: this.recordData.product_height,
                default_product_depth: this.recordData.product_depth,
            },
            this.dataPointID
        );
...
//                ),
                ...
                species_ids: changed ? [] : this._getAttributeValueIds(
                    data.species_ids
                ),
    ...
    _onAddRootProductOnly: function () {
        if (this.renderer.productWidth < this.renderer.productMinWidth){
            alert("Kindly enter more size than min size of width");
            return false;
        }
        if (this.renderer.productWidth > this.renderer.productMaxWidth){
            alert("Kindly enter min size than max size of width");
            return false;
        }
...
//        this.rootProduct['material'] = this.renderer.materialProducts;
        ...
        this.rootProduct['product_width'] = this.renderer.productWidth;
        this.rootProduct['product_height'] = this.renderer.productHeight;
        this.rootProduct['product_depth'] = this.renderer.productDepth;
...
//        products[0]['material'] = this.renderer.materialProducts;
        ...
        products[0]['product_width'] = this.renderer.productWidth;
        products[0]['product_height'] = this.renderer.productHeight;
        products[0]['product_depth'] = this.renderer.productDepth;
        this._super.apply(this, arguments);
    },
});
});
