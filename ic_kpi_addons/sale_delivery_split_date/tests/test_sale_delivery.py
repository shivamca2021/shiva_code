...
class TestSaleDelivery(TransactionCase):
    ...
    def test_check_single_date(self):
        self.assertEqual(len(self.so.picking_ids), 0, "There must not be pickings for the SO when draft")
        self.so.action_confirm()
        self.assertEqual(len(self.so.picking_ids), 1, "There must be 1 picking for the SO when confirmed")
        self.assertEqual(self.so.picking_ids[0].scheduled_date.date(), self.so.picking_ids[0].min_dt)
        self.assertEqual(
            self.so.picking_ids[0].scheduled_date, self.date_sooner, "The picking must be planned at the expected date"
        )
        self.assertEqual(
        ...
        self.assertIn(format_date(self.env, self.date_sooner.date()), self.so_line1.procurement_group_id.name)
    ...
    def test_check_multiple_dates(self):
        ...
        self.assertEqual(len(self.so.picking_ids), 0, "There must not be pickings for the SO when draft")
        self.so.action_confirm()
        self.assertEqual(len(self.so.picking_ids), 2, "There must be 2 pickings for the SO when confirmed")
        sorted_pickings = self.so.picking_ids.sorted(lambda x: x.scheduled_date)
        self.assertEqual(self.so.picking_ids[0].scheduled_date.date(), self.so.picking_ids[0].min_dt)
        self.assertEqual(
            sorted_pickings[0].scheduled_date, self.date_sooner, "The first picking must be planned at the soonest date"
        )
        self.assertEqual(self.so.picking_ids[1].scheduled_date.date(), self.so.picking_ids[1].min_dt)
        self.assertEqual(
            sorted_pickings[1].scheduled_date, self.date_later, "The second picking must be planned at the latest date"
        )
        self.assertNotEqual(
            self.so_line1.procurement_group_id,
            self.so_line2.procurement_group_id,
        ...
        self.assertIn(format_date(self.env, self.date_sooner.date()), self.so_line1.procurement_group_id.name)
        self.assertIn(format_date(self.env, self.date_later.date()), self.so_line2.procurement_group_id.name)
    ...
    def test_check_same_dates(self):
        ...
        self.assertEqual(len(self.so.picking_ids), 0, "There must not be pickings for the SO when draft")
        self.so.action_confirm()
        self.assertEqual(len(self.so.picking_ids), 1, "There must be only one picking for the SO when confirmed")
        self.assertEqual(self.so.picking_ids[0].scheduled_date.date(), self.so.picking_ids[0].min_dt)
        self.assertEqual(
            self.so.picking_ids.scheduled_date, self.date_sooner, "The picking must be planned at the expected date"
        )
        self.assertEqual(
            self.so_line1.procurement_group_id,
            self.so_line2.procurement_group_id,
